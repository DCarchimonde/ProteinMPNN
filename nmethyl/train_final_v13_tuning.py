...
# 3. 损失函数 (平衡策略)
# =============================================================================
def calculate_balanced_loss(logits_base, logits_methyl, targets, mask, methyl_weight=2.0):
    ...
    # Base Head: 标准 CrossEntropy (不加 Smoothing，相信预训练的硬判断)
    loss_base = F.cross_entropy(logits_base, base_targets, ignore_index=-100)
    
    # Methyl Head: CrossEntropy with weight
    loss_methyl = F.cross_entropy(
        logits_methyl, 
        methyl_targets, 
        weight=torch.tensor([1.0, 3.0], device=logits_base.device) # 降低负样本权重
    )
...
def validate(model, loader, device, verbose=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_is_methyl = (probs_methyl > 0.4).long() # [调参] 提高阈值到0.4，减少假阳性
...
def final_evaluation(model, loader, device):
    model.eval()
    print("\n" + "="*50)
    print("📊 FINAL EVALUATION REPORT (Optimization V13)")
    print("="*50)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v13_tuning")
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--methyl_loss_weight", type=float, default=2.0) # [调参] 降权
    ...
    model = DecoupledProteinMPNN(augment_eps=0.3).to(device) # [调参] 增加噪声 augment_eps=0.3
    ...
    # [核心调参] 极低的主干学习率，保护预训练权重
    optimizer = torch.optim.AdamW([
        {'params': [p for n, p in model.named_parameters() if 'W_out' not in n], 'lr': 1e-6}, # Backbone Locked
        {'params': [p for n, p in model.named_parameters() if 'W_out' in n], 'lr': 5e-4}      # Head Boosted
    ], weight_decay=1e-4)
    
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=10, verbose=True)

    print("Starting Training (V13 - Conservative Backbone)...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            X, S, mask, chain_M, residue_idx, chain_encoding_all = f
            lb, lm = model(X, S, mask, chain_M, residue_idx, chain_encoding_all)
            loss, valid = calculate_balanced_loss(lb, lm, S, mask, methyl_weight=args.methyl_loss_weight)
        ...
        if epoch >= 50 and epoch % 10 == 0: # 更频繁地保存 Soup
