...
def compute_hbond_mask(X, mask):
    ...
    N_atoms = X[:, :, 0, :] 
    O_atoms = X[:, :, 3, :]
    dists = torch.cdist(N_atoms, O_atoms) # [B, L, L]
...
# =============================================================================
# V25 Model: Soft Physics Awareness
# =============================================================================
class SoftPhysicsProteinMPNN(ProteinMPNN):
    def __init__(self, hidden_dim=128, augment_eps=0.1, **kwargs):
        ...
        self.W_out_methyl = nn.Linear(hidden_dim, 1)
...
def smart_load_weights(model, pretrained_path, device):
    print(f"\n>>> [V25 Init] Loading {pretrained_path}...")
...
def calculate_loss_v25(logits_base, logits_methyl, targets, mask):
    ...
    logits_methyl = logits_methyl.contiguous().view(-1)[mask_flat][valid_pos]
    ...
    loss_methyl = F.binary_cross_entropy_with_logits(
        logits_methyl, binary_targets, pos_weight=torch.tensor([8.0], device=logits_base.device)
    )

    return loss_base + 4.0 * loss_methyl, base_targets.numel()

# Dataset helpers (Standard)
...
def validate_and_report(model, loader, device, epoch_num):
    ...
    # 用于统计详细表格
    methyl_stats = {} 
    for m_aa in METHYL_AA_ALPHABET:
        methyl_stats[m_aa] = {"count": 0, "correct": 0, "miss_nat": 0, "miss_other": 0}
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            prob_methyl = torch.sigmoid(l_methyl.squeeze(-1))
            
            # 软惩罚：如果有氢键，概率 - 0.3 (而不是直接清零)
            penalty = 0.3
            final_score = prob_methyl - (has_h_bond.float() * penalty)
            is_methyl_final = final_score > 0.5
            
            # --- Override Logic ---
            final_pred = pred_base_idx.clone()
            for nat_idx, met_idx in nat_idx_to_methyl_idx.items():
                mask_nat = (pred_base_idx == nat_idx) & is_methyl_final
            ...
            for idx in valid_indices:
                ...
                if t >= len(NATURAL_AA_ALPHABET):
                    ...
                    if p == t:
                        ...
                        # 检查是不是预测成了对应的天然氨基酸
                        natural_char = NMETHYL_TO_NATURAL_MAPPING.get(METHYL_AA_ALPHABET.index(t_char), -1)
                        if natural_char != -1:
                            nat_idx = NATURAL_AA_ALPHABET.index(NATURAL_AA_ALPHABET[natural_char])
                            if p == nat_idx:
    ...
    # --- 打印漂亮的表格 ---
    print(f"\n🧪 [V25 Soft-Physics] Epoch {epoch_num} Report:")
    print("-" * 60)
    ...
    for aa in sorted(methyl_stats.keys()):
        s = methyl_stats[aa]
        if s["count"] > 0:
            rec = (s["correct"] / s["count"]) * 100
            print(f"{aa:<10} {s['count']:<8} {rec:6.2f}%    {s['miss_nat']:<10} {s['miss_other']:<10}")
    
    print("-" * 60)
    print(f"🔥 Overall Methyl Recall: {methyl_recall*100:.2f}%")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v25_soft")
    ...
    model = SoftPhysicsProteinMPNN(augment_eps=0.1).to(device)
    ...
    print("🚀 Starting V25 Soft-Physics Run (With Detailed Report)...")
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            X, S, mask, chain_M, residue_idx, chain_encoding_all = f
            l_base, l_methyl = model(X, S, mask, chain_M, residue_idx, chain_encoding_all)
            loss, _ = calculate_loss_v25(l_base, l_methyl, S, mask)
            loss.backward()
            optimizer.step()
