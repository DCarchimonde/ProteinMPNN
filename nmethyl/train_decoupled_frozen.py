...
def featurize_batch(batch, device):
    ...
    for b in batch:
        ...
        lengths.append(l)
    L_max = max(lengths)
    X = np.zeros([B, L_max, 4, 3]); S = np.zeros([B, L_max], dtype=np.int32); mask = np.zeros([B, L_max], dtype=np.float32)
    chain_M = np.zeros([B, L_max], dtype=np.float32); residue_idx = -100 * np.ones([B, L_max], dtype=np.int32); chain_encoding_all = np.zeros([B, L_max], dtype=np.int32)
    for i, b in enumerate(batch):
        all_chains = sorted(list(set([k.split('_')[-1] for k in b.keys() if k.startswith('seq_chain_')])))
        l_p = 0
        for c_i, c_id in enumerate(all_chains):
            seq = b.get(f'seq_chain_{c_id}', '')
            N = np.array(b.get(f'N_chain_{c_id}', [])); CA = np.array(b.get(f'CA_chain_{c_id}', [])); C = np.array(b.get(f'C_chain_{c_id}', [])); O = np.array(b.get(f'O_chain_{c_id}', []))
            l = min(len(seq), len(CA))
            if l == 0: continue
            X[i, l_p:l_p+l, 0, :] = N[:l]; X[i, l_p:l_p+l, 1, :] = CA[:l]; X[i, l_p:l_p+l, 2, :] = C[:l]; X[i, l_p:l_p+l, 3, :] = O[:l]
            indices = []
            for aa in seq[:l]: indices.append(EXTENDED_AA_TO_INDEX.get(aa, EXTENDED_AA_TO_INDEX['X']))
            S[i, l_p:l_p+l] = indices
            chain_M[i, l_p:l_p+l] = 1.0; residue_idx[i, l_p:l_p+l] = np.arange(l) + c_i * 100; chain_encoding_all[i, l_p:l_p+l] = c_i
            l_p += l
...
def final_evaluation(model, loader, device):
    model.eval()
    print("\n" + "="*50)
    print("📊 FINAL EVALUATION REPORT (FROZEN BACKBONE)")
    print("="*50)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            targets = S.cpu().numpy().flatten(); preds_b = pred_base_idx.cpu().numpy().flatten(); preds_m = pred_is_methyl.cpu().numpy().flatten(); preds_final = final_pred.cpu().numpy().flatten(); mask_flat = mask.cpu().numpy().flatten().astype(bool)
            ...
            all_targets_base.extend(batch_targets_base); all_preds_base.extend(preds_b[valid])
            all_targets_methyl.extend(batch_targets_methyl); all_preds_methyl.extend(preds_m[valid])
            all_targets_combined.extend(batch_targets); all_preds_combined.extend(preds_final[valid])
    base_acc = np.mean(np.array(all_targets_base) == np.array(all_preds_base))
    methyl_acc = np.mean(np.array(all_targets_methyl) == np.array(all_preds_methyl))
    total_acc = np.mean(np.array(all_targets_combined) == np.array(all_preds_combined))
    print(f"\n✅ Base AA Accuracy (Ignoring Methylation): {base_acc*100:.2f}%")
    print(f"   (不考虑甲基化，只看氨基酸种类是否正确 -> 预期接近 50%)")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_frozen_backbone")
    parser.add_argument("--epochs", type=int, default=50) # 冻结后不需要太多轮
    ...
    if 'W_s.weight' in state_dict:
        pt_W_s = state_dict['W_s.weight']
        model.W_s.weight.data[:21, :] = pt_W_s[:21, :]
        del state_dict['W_s.weight']
        print(" [Info] Solved W_s mismatch: Copied first 21 embeddings.")
    model.load_state_dict(state_dict, strict=False) 

    # ❄️❄️❄️ 核心修改：冻结主干 ❄️❄️❄️
    print("❄️ Freezing Backbone (Encoder/Decoder/Embeddings)...")
    for name, param in model.named_parameters():
        if "W_out_methyl" in name: # 只训练甲基化头
            param.requires_grad = True
        elif "W_out_base" in name: # 可选：也微调一下基础头，或者冻结它
            param.requires_grad = True 
        else:
            param.requires_grad = False # 冻结其他所有！
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save({'model_state_dict': model.state_dict()}, os.path.join(args.output_dir, "best_model_frozen.pt"))

    print(">>> Loading best frozen model for final evaluation...")
    model.load_state_dict(torch.load(os.path.join(args.output_dir, "best_model_frozen.pt"))['model_state_dict'])
