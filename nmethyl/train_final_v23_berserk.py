...
class DecoupledProteinMPNN(nn.Module):
    def __init__(self, num_letters=21, node_features=128, edge_features=128,
                 ...
                 vocab=21, k_neighbors=48, augment_eps=0.3, dropout=0.1): # [狂暴] 噪声提升到 0.3
...
def advanced_load_weights(model, pretrained_path, device):
    ...
    print("✅ Weights Loaded (Standard Architecture - Loose Match + Smart Init).")

# =============================================================================
# 4. 损失函数
# =============================================================================
def calculate_loss(logits_base, logits_methyl, targets, mask, methyl_weight=10.0): # [狂暴] 回归 10.0
    ...
    # [微调] Base加上0.1 Smoothing 防止过拟合
    loss_base = F.cross_entropy(logits_base, base_targets, label_smoothing=0.1, ignore_index=-100)
    loss_methyl = F.cross_entropy(logits_methyl, methyl_targets, weight=torch.tensor([1.0, 5.0], device=logits_base.device)) # 提高正样本权重
...
def get_weighted_sampler(dataset, oversample_weight=50.0): # [狂暴] 回归 50.0
    weights = []
    offset = len(NATURAL_AA_ALPHABET)
...
def validate(model, loader, device, verbose=False):
    ...
    t_comb = np.array(all_targets_comb); p_comb = np.array(all_preds_comb)
...
def final_report(model, loader, device):
    model.eval()
    print("\n" + "="*50)
    print("📊 FINAL EVALUATION REPORT (V23 BERSERK)")
    print("="*50)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v23_berserk")
    parser.add_argument("--epochs", type=int, default=150)
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--methyl_loss_weight", type=float, default=10.0) # [狂暴] 10.0
    parser.add_argument("--sampler_weight", type=float, default=50.0) # [狂暴] 50.0
    ...
    model = DecoupledProteinMPNN(augment_eps=0.3).to(device) # [狂暴] 噪声 0.3
    ...
    optimizer = torch.optim.AdamW([
        {'params': [p for n, p in model.named_parameters() if 'W_out' not in n], 'lr': 1e-4},
        {'params': [p for n, p in model.named_parameters() if 'W_out' in n], 'lr': 1e-3}
    ], weight_decay=1e-4)
    ...
    print("Starting Training (V23 - Berserk Mode)...")
    best_total_acc = 0.0
