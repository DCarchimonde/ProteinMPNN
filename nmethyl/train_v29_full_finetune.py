...
class ProteinMPNN_DualHead(nn.Module):
    def __init__(self, hidden_dim=128, k_neighbors=48):
        ...
        # HEAD 1: Base Amino Acid Prediction (20 classes)
        ...
        # HEAD 2: Methylation Prediction (2 classes)
        # 
        self.head_methyl = nn.Sequential(
    ...
    def forward(self, inputs):
        ...
        # 1. Base Logits [B, L, 20]
        logits_base = self.head_base(h_V) 
        
        # 2. Methyl Logits [B, L, 2]
        # (BatchNorm requires 2D input)
        logits_methyl = self.head_methyl(h_V_flat).view(B, L, 2)
    ...
    for i, b in enumerate(batch):
        ...
        for char in seq:
            ...
            if idx >= 21: 
                ...
                mpnn_seq.append(base_idx) 
                base_labels.append(base_idx)
...
def main():
    ...
    # 1. Model
    model = ProteinMPNN_DualHead(hidden_dim=128, k_neighbors=48).to(device)
    
    # 2. Load Weights (Backbone Only)
    ...
    # 3. Setup Optimizer (UNFREEZE BACKBONE!)
    # Backbone: Low LR (Fine-tune) to fix the 5% Base Acc issue
    # Heads: High LR (Learn new tasks)
    optimizer = torch.optim.AdamW([
        {'params': model.features.parameters(), 'lr': 1e-6},
        {'params': model.encoder_layers.parameters(), 'lr': 1e-6},
        {'params': model.W_e.parameters(), 'lr': 1e-6},
        {'params': model.W_s.parameters(), 'lr': 1e-6},
        {'params': model.head_base.parameters(), 'lr': 1e-3},
        {'params': model.head_methyl.parameters(), 'lr': 1e-3}
    ], weight_decay=1e-4)
    ...
    for ep in range(50):
        ...
        for batch in loader:
            ...
            # Losses
            loss_base = F.cross_entropy(logits_base.view(-1, 20), y_base.view(-1))
    ...
    # Auto-find best threshold for Methyl
    threshold = 0.3
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # 1. Predict Base
            pred_base = torch.argmax(F.softmax(logits_base, -1), -1) # 0-19
            ...
            pred_methyl = (probs_methyl > threshold).long() # 0/1
            ...
            for b in range(B):
                for l in range(L):
                    ...
                    if mask[b,l] == 1 and pred_methyl[b,l] == 1:
                        methyl_idx = base_idx + 21
                        if methyl_idx <= 40:
                            final_seq[b,l] = methyl_idx
    ...
    base_mask = t < 20
    if base_mask.sum() > 0:
        print(f"Base Acc:   {np.mean(t[base_mask]==p[base_mask])*100:.2f}%")
