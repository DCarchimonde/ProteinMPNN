...
METHYL_AA_ALPHABET = "acdefghiklmnqrstvwvy" 
...
def gather_nodes(nodes, neighbor_idx):
    # Features [B,N,C] at Neighbor indices [B,N,K]
...
class EncLayer(nn.Module):
    ...
    def forward(self, h_V, h_E, E_idx, mask_V=None, mask_attend=None):
        # [修复] 必须先 Gather 邻居节点特征
        h_V_neigh = gather_nodes(h_V, E_idx)
        h_V_expand = h_V.unsqueeze(-2).expand(-1, -1, h_E.size(-2), -1)
        
        # Block 1: [h_V, h_V_neigh, h_E] -> 128 + 128 + 128 = 384
        ...
        # Block 2: Re-gather neighbors because h_V updated
        h_V_neigh = gather_nodes(h_V, E_idx)
        h_V_expand = h_V.unsqueeze(-2).expand(-1, -1, h_E.size(-2), -1)
        h_EV = torch.cat([h_V_expand, h_V_neigh, h_E], -1)
...
class DecLayer(nn.Module):
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None):
        # [修复] h_E 在这里实际上是 [B, L, K, 384] 的邻居特征组合
        # 我们需要把 h_V 扩展并拼接上去 -> 128 + 384 = 512
        h_V_expand = h_V.unsqueeze(-2).expand(-1, -1, h_E.size(-2), -1)
...
def featurize_batch(batch, device):
    ...
    for i, b in enumerate(batch):
        ...
        # ⚠️ Physics Fix: Swap CA/O if needed
    ...
    # ⚠️ Scale Fix
    if X.shape[1] > 1:
        valid_ca = X[0, :, 1, :][mask[0] > 0]
        if len(valid_ca) > 1:
            avg_dist = np.mean(np.linalg.norm(valid_ca[1:] - valid_ca[:-1], axis=-1))
            if avg_dist < 3.2 and avg_dist > 0.1:
                X *= (3.8 / avg_dist)
                print(f"⚠️ [Physics] Auto-scaled structure (x{3.8/avg_dist:.2f}).")
...
def validate(model, loader, device, verbose=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            all_preds_base.extend(pb.cpu().numpy().flatten()[valid])
            all_targets_base.extend(t_base)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v16_stable")
    ...
    optimizer = torch.optim.AdamW([
        {'params': [p for n, p in model.named_parameters() if 'W_out' not in n], 'lr': 1e-5},
    ...
    print("Starting Training (V16 - Stable)...")
    best_total_acc = 0.0
