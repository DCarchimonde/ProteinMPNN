...
def evaluate_binary_sampling(model, loader, device):
    model.eval()
    print("\n>>> 正在进行 [二分类] 甲基化位点探测评估 (Binary Site Detection)...")
    
    y_true_binary = [] # 0: 非甲基化, 1: 甲基化
    y_pred_binary = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # --- [High Temperature Sampling] 模拟多样性生成 ---
            # 我们不做复杂的采样，直接看 logits_methyl 的概率
            logits_base, logits_methyl = model(X, S, mask, chain_M, residue_idx, chain_encoding_all)
            
            # logits_methyl: [B, L, 2] -> index 1 是甲基化概率
            probs_methyl = F.softmax(logits_methyl, dim=-1)[:, :, 1] # 取出甲基化概率
            
            # 真实标签
            is_methyl_true = (S >= len(NATURAL_AA_ALPHABET)).long()
            
            # 展平处理
            valid = mask.bool() & (S != EXTENDED_AA_TO_INDEX.get('X', -1))
            
            y_true_binary.extend(is_methyl_true[valid].cpu().numpy())
            y_pred_binary.extend((probs_methyl[valid] > 0.5).long().cpu().numpy()) # 阈值 0.5
    ...
    print("\n" + "="*50)
    print(f"🔥 NUCLEAR OPTION: BINARY METHYLATION DETECTION 🔥")
    print("="*50)
    print(f"Binary Accuracy:        {acc:.4f} ({acc*100:.2f}%)")
    print("-" * 50)
    print("Classification Report (0=No Methyl, 1=Methyl):")
    print(classification_report(y_true_binary, y_pred_binary, target_names=["Non-Methyl", "Methylified"], zero_division=0))
    print("-" * 50)
...
if __name__ == "__main__":
    ...
    evaluate_binary_sampling(model, loader, device)