...
class ProteinMPNN(nn.Module):
    def __init__(self, hidden_dim=128, k_neighbors=48):
        ...
        self.W_s = nn.Embedding(21, hidden_dim) # 21 standard vocab
...
# =============================================================================
# 3. 甲基化专用分类头 (Methyl Head)
# =============================================================================
class MethylClassifier(nn.Module):
    def __init__(self, input_dim=128):
        super().__init__()
        self.backbone = ProteinMPNN(hidden_dim=128, k_neighbors=48)
    ...
    def forward(self, inputs_tuple):
        X, S, mask, residue_idx, chain_encoding_all = inputs_tuple
        
        # 冻结 backbone 推理
        with torch.no_grad():
            h_V = self.backbone(X, S, mask, residue_idx, chain_encoding_all)
...
def main():
    ...
    print(f"✅ Backbone Loaded. Missing keys (Expected Decoder keys): {len(msg.missing_keys)}")
    ...
    print(">>> Training Binary Classifier (Backbone Frozen)...")
    for ep in range(50):
        model.train()
        total_loss = 0
        for batch in loader:
        ...
        if ep % 5 == 0:
            print(f"Ep {ep}: Focal Loss {total_loss/len(loader):.4f}")
            
    # ... (前面的代码不变，替换 main 函数最后的 Inference 部分) ...

    print("\n>>> 🔍 Threshold Scanning (Finding the Sweet Spot)...")
    model.eval()
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            logits = model(inputs)
            # 获取属于 Class 1 (Methyl) 的原始概率
            probs = F.softmax(logits, dim=-1)[:,:,1] 
            
            valid = mask.bool()
            all_probs.extend(probs[valid].cpu().numpy())
            all_true.extend(targets[valid].cpu().numpy())
            
    t = np.array(all_true)
    probs = np.array(all_probs)
    
    print(f"{'Threshold':<10} | {'Accuracy':<10} | {'Recall':<10} | {'Precision':<10} | {'F1 Score':<10}")
    print("-" * 60)
    ...
    for threshold in [0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99]:
        ...
        print(f"{threshold:<10.2f} | {acc:<10.2f}% | {recall:<10.2f}% | {precision:<10.2f}% | {f1:<10.2f}")
