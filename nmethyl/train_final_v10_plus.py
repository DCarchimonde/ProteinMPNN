...
# =============================================================================
# 2. 权重加载器 (v10 Parent Init)
# =============================================================================

def advanced_load_weights(model, pretrained_path, device):
    print(f"\n>>> [v10+ Loader] Opening {pretrained_path}...")
...
# --- [关键] TTA 评估函数 ---
def final_evaluation_tta(model, loader, device, tta_steps=10):
    model.eval()
    print(f"\n=== Final Evaluation (TTA Mode: {tta_steps} repeats) ===")
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # TTA: 跑多次，累加 logits
            accum_logits_base = 0
            accum_logits_methyl = 0
            
            for t in range(tta_steps):
                # 在每次 forward 时，ProteinMPNN 的 features 模块会自动加上 augment_eps 的噪声
                # 前提是 model.train() 模式？不，我们可以手动控制。
                # 或者我们手动给 X 加噪声。
                X_noisy = X + 0.1 * torch.randn_like(X) # 手动加噪声 0.1 Angstrom
                lb, lm = model(X_noisy, S, mask, chain_M, residue_idx, chain_encoding_all)
                accum_logits_base += lb
                accum_logits_methyl += lm
            
            # 取平均
            lb = accum_logits_base / tta_steps
            lm = accum_logits_methyl / tta_steps
    ...
    print(f"Overall Accuracy (TTA): {acc:.4f}")
    print(f"Natural AA Recovery (TTA): {natural_acc:.4f}")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./final_run_v10_plus")
    parser.add_argument("--epochs", type=int, default=300) # 增加 epoch
    ...
    parser.add_argument("--warmup_epochs", type=int, default=5) # 冻结阶段
    ...
    print(f"\n>>> Stage 1: Warmup (Freezing Backbone for {args.warmup_epochs} epochs)...")
    for name, param in model.named_parameters():
        if "W_out" not in name and "W_s" not in name:
    ...
    for epoch in range(1, args.warmup_epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss, valid = calculate_final_loss(lb, lm, S, mask, methyl_weight=args.methyl_loss_weight)
            if valid > 0 and torch.isfinite(loss):
                loss.backward()
                optimizer.step()
        print(f"  Warmup Epoch {epoch} Done.")

    # --- Stage 2: Unfreeze & Full Finetune ---
    print(f"\n>>> Stage 2: Full Finetuning (Unfreezing All)...")
    ...
    # T_0=50 (每50个epoch重启一次学习率), T_mult=2 (周期翻倍)
    ...
    # --- Final Eval with TTA ---
    print("\nTraining Complete. Loading best model for TTA Evaluation...")
    model.load_state_dict(torch.load(os.path.join(args.output_dir, "best_model.pt"))['model_state_dict'])
    final_evaluation_tta(model, test_loader, device, tta_steps=10)
