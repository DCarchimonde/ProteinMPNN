...
# Base Index (0-19) -> Methyl Index (21-40)
...
# =============================================================================
# 2. 模型定义 (V32: 512-Dim Fixed)
# =============================================================================
def gather_nodes(nodes, neighbor_idx):
    # Robust gather function
...
class DecLayer(nn.Module):
    def __init__(self, hidden_dim, dropout=0.1):
        ...
        # [V32 FIX] 128*4 = 512. MATCHES CHECKPOINT [128, 512]
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None):
        # h_V: [B, L, 128] -> h_V_expand: [B, L, K, 128]
        h_V_expand = h_V.unsqueeze(-2).expand(-1,-1,h_E.size(-2),-1)
        # h_E: [B, L, K, 384] (Context)
        # Concat: 128 + 384 = 512 -> Linear(512, 128)
...
class ProteinMPNN_Dual(nn.Module):
    ...
    def forward(self, X, S, mask, residue_idx, chain_encoding_all):
        ...
        # [V32 CONTEXT: 384 DIMENSION]
        # h_E (128) + h_S_neigh (128) + h_V_neigh (128) = 384
        h_S_neigh = gather_nodes(h_S, E_idx)
        h_V_neigh = gather_nodes(h_V, E_idx)
        # The concatenation order that worked in V22 was likely [E, S, V] or similar
        # Let's use: E, S, V
        ...
        mask_attend = gather_nodes(order_mask_backward.unsqueeze(-1), E_idx).squeeze(-1)
        mask_1D = mask.view([mask.size(0), mask.size(1), 1, 1])
        mask_bw = mask_1D * mask_attend
        
        for layer in self.decoder_layers:
            # Mask context
            h_Context_masked = mask_bw * h_Context
            # Pass to layer (internally +128 self = 512)
            h_V = layer(h_V, h_Context_masked, mask, mask_attend=mask_bw)
...
def main():
    ...
    # Expected missing keys: heads (W_out_base, W_out_methyl) are initialized randomly or partially if names match
    
    # 3. Train
    # Use standard CrossEntropy for Base, Focal for Methyl
    criterion_methyl = FocalLoss(alpha=0.25, gamma=2.0)
    
    # Optimizer: Fine-tune backbone slowly, heads fast
    optimizer = torch.optim.AdamW([
        {'params': model.features.parameters(), 'lr': 1e-6},
        {'params': model.encoder_layers.parameters(), 'lr': 1e-6},
        {'params': model.decoder_layers.parameters(), 'lr': 1e-6},
    ...
    print(">>> Training Dual Model (Base + Methyl)...")
    for ep in range(50):
        model.train()
        total_loss = 0
        for batch in loader:
        ...
        for batch in test_loader:
            ...
            for b in range(B):
                for l in range(L):
                    ...
                    # Only modify if methyl is predicted AND we have a mapping
