...
class DecoupledProteinMPNN(nn.Module):
    def __init__(self, num_letters=21, node_features=128, edge_features=128,
                 ...
                 vocab=21, k_neighbors=48, augment_eps=0.2, dropout=0.1): # [改回] 0.2
...
def featurize_batch(batch, device):
    ...
    for i, b in enumerate(batch):
        ...
        # ⚠️ Physics Fix: Swap CA/O if needed (保留这个基本纠错)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v18_rescue")
    ...
    model = DecoupledProteinMPNN(augment_eps=0.2).to(device) # [改回] 0.2
    ...
    # [核心调参] 
    # W_out_base (Base Head): 1e-4 -> 保护老知识
    # W_out_methyl (Methyl Head): 1e-3 -> 猛练新知识
    # Backbone: 1e-4 -> 保持灵活性
    
    base_head_params = list(map(id, model.W_out_base.parameters()))
    methyl_head_params = list(map(id, model.W_out_methyl.parameters()))
    backbone_params = filter(lambda p: id(p) not in base_head_params and id(p) not in methyl_head_params, model.parameters())

    optimizer = torch.optim.AdamW([
        {'params': backbone_params, 'lr': 1e-4}, 
        {'params': model.W_out_base.parameters(), 'lr': 1e-4}, # Low LR for Base
        {'params': model.W_out_methyl.parameters(), 'lr': 1e-3} # High LR for Methyl
    ], weight_decay=1e-4)
    
    scheduler = OneCycleLR(optimizer, max_lr=[1e-4, 1e-4, 1e-3], epochs=args.epochs, steps_per_epoch=len(train_loader), pct_start=0.3, div_factor=10)

    print("Starting Training (V18 - Rescue Mode)...")
    best_total_acc = 0.0
