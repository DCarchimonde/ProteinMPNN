...
# =============================================================================
# V26 Model: Binary Decoupled (No Physics)
# =============================================================================
class BinaryDecoupledProteinMPNN(ProteinMPNN):
...
def smart_load_weights(model, pretrained_path, device):
    print(f"\n>>> [V26 Init] Loading {pretrained_path}...")
...
def calculate_loss_v26(logits_base, logits_binary, targets, mask):
    ...
    # 2. Binary Loss (Methylation Flag)
    binary_targets = (targets_flat >= len(NATURAL_AA_ALPHABET)).float()
    
    # pos_weight=10.0 让模型极度重视那稀少的甲基化样本
...
def validate_and_report(model, loader, device, epoch_num, output_dir):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # --- PURE LEARNING: No Physics ---
            # 直接信 Binary Head，阈值 0.5 (或者更低，这里用 0.5 试试看)
            is_methyl = (torch.sigmoid(l_binary.squeeze(-1)) > 0.5)
            ...
            for idx in valid_indices:
                t = tgts[idx]
                p = preds[idx]
                if t >= len(NATURAL_AA_ALPHABET): # 只统计甲基化样本
    ...
    # 我们用 Recall 作为 Score，因为这是你最关心的
    current_score = methyl_recall 

    # --- 打印报表 (完美复刻) ---
    print(f"\n🧐 Detailed Binary-Decoupled Report (V26 Pure):")
    print(f"{'Methyl AA':<10} {'Count':<8} {'Recall':<10} {'Miss->Nat':<10} {'Miss->Other':<10}")
    print("-" * 60)
    
    sorted_keys = sorted(methyl_stats.keys())
    for aa in sorted_keys:
        s = methyl_stats[aa]
        rec_str = f"{(s['correct']/s['count'])*100:.1f}%" if s["count"] > 0 else "0.0%"
        print(f"{aa:<10} {s['count']:<8} {rec_str:<10} {s['miss_nat']:<10} {s['miss_other']:<10}")
        
    print("-" * 60)
    print(f"🏆 Binary Score: {current_score:.4f} (BaseAcc: {base_acc:.4f}, MeRec: {methyl_recall:.4f})")
    
    # 保存最佳模型逻辑
    if current_score > best_score:
        best_score = current_score
        save_path = os.path.join(output_dir, f"best_model_v26_score_{best_score:.4f}.pt")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v26_pure")
    ...
    model = BinaryDecoupledProteinMPNN(augment_eps=0.1).to(device)
    ...
    print("🚀 Starting V26 Pure Run (Binary Decoupled, No Physics, Full Report)...")
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            X, S, mask, chain_M, residue_idx, chain_encoding_all = f
            l_base, l_binary = model(X, S, mask, chain_M, residue_idx, chain_encoding_all)
            loss, _ = calculate_loss_v26(l_base, l_binary, S, mask)
            loss.backward()
            optimizer.step()
            epoch_loss += loss.item()
            batches += 1
            
        print(f"Epoch {epoch}: Loss = {epoch_loss/batches:.4f}")
