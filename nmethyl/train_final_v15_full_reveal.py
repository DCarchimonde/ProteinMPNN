...
def smart_load_weights(model, pretrained_path, device):
    print(f"\n>>> [V15 Init] Loading {pretrained_path} with SMART MAPPING...")
        ...
        with torch.no_grad():
            ...
            for nme_idx, nat_idx in NMETHYL_TO_NATURAL_MAPPING.items():
                if nme_idx < len(METHYL_AA_ALPHABET):
                    ...
                    # 获取该天然氨基酸在当前模型中的 Embedding (此时已经是预训练的值了)
...
def calculate_loss_v15(logits_base, logits_methyl, targets, mask):
    ...
    # pos_weight=5.0 意味着漏报甲基化的代价是误报的 5 倍
    # 结合外面的 multiplier，总惩罚力度非常大
...
def validate_and_report(model, loader, device, epoch_num):
    ...
    # True Positive Methyl (既要是甲基化，又要猜对是谁)
    ...
    # 返回 Total Accuracy 作为保存标准 (虽然现在我们更看重 Recall，但总 Acc 还是综合指标)
    # 或者，我们可以返回一个加权分数：0.5 * BaseAcc + 0.5 * MethylRecall
    ...
    print(f"🏆 Composite Score: {composite_score:.4f} (BaseAcc: {base_acc:.4f}, MeRec: {methyl_recall:.4f})")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./run_v15_reveal")
    ...
    sampler = get_weighted_sampler(train_ds, oversample_weight=10.0) # 采样加权
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            # 使用 V15 Loss
            loss, valid = calculate_loss_v15(lb, lm, S, mask)
        ...
        # 每 5 个 Epoch 做一次全面体检 (或者你可以改成 2，如果你很急)
        if epoch % 5 == 0:
            current_score = validate_and_report(model, test_loader, device, epoch)
            
            if current_score > best_score:
                best_score = current_score
                save_path = os.path.join(args.output_dir, "best_model_v15.pt")
                torch.save({'model_state_dict': model.state_dict()}, save_path)
                print(f"🌟 New Best Model Saved! (Composite Score: {best_score:.4f})")
